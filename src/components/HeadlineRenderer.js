...
class HeadlineRenderer extends Component {
  render() {
    return (
      <div>
        {articles[this.props.newsSource].map(article => <NewsHeadline image={article.urlToImage} headline={article.title} description={article.description} articleLink={article.url}/>)}
      </div>
    );
  }
}
